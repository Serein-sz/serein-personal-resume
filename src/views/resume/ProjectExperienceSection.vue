...
<template>
  <div class="project-experience-section">
    <h2 class="text-2xl font-semibold mb-2">项目经历</h2>
    <div v-for="(project, index) in projectExperiences" :key="index" class="mb-6 p-4 border border-gray-200 rounded-lg shadow-sm dark:border-gray-700">
      ...
      <div class="mb-2">
        <h4 class="font-medium text-gray-800 dark:text-gray-200">核心技术栈:</h4>
        <div class="flex flex-wrap gap-2 mt-1">
          <span v-for="(tech, techIndex) in project.techStack" :key="techIndex" class="px-2 py-1 bg-blue-100 text-blue-800 text-xs font-medium rounded-full dark:bg-blue-900 dark:text-blue-300">
            {{ tech }}
          </span>
        </div>
      </div>
      <div>
        <h4 class="font-medium text-gray-800 dark:text-gray-200">职责与贡献:</h4>
        <ul class="list-disc list-inside ml-4 mt-1 space-y-1 text-gray-700 dark:text-gray-300">
          <li v-for="(resp, respIndex) in project.responsibilities" :key="respIndex">{{ resp }}</li>
        </ul>
      </div>
    </div>
  </div>
